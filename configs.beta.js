//测试环境
var configs = {
  name: 'Steward.Web',
  version: '1.0.0-beta.0',
  productName: '福禄管家-系统总管',
  productVersion: 'X1',
  clientId:'10000003',
  accessToken:'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..cfYs3h1KmMdExHPNevec1s2CLOSEcAuN1aIXiTiouhc',
  host: {
    passport:{
      'getUserInfo': 'http://10.0.0.138:8087',//获取通行证
      'authCode': 'http://10.0.0.189:10080', //获取通行证
      'auth': 'http://10.0.0.138:8090'// 通行证登录页
    },
    common:'http://10.0.0.189:10080',//左侧导航和头部导航 福禄管家通用接口
    steward:'http://10.0.0.189:10080',//系统管家 接口
    webapi:'http://10.0.0.189:10081' //新接口的地址
  }
};
