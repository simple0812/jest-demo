...
class CommonPage extends React.Component {
  ...
  //为了兼容各种enabled字段，需要传入key 默认为enabled
  handleToggleEnable = (record, enabledKey) => {
    enabledKey = enabledKey ||  'enabled';
    record[enabledKey] = !record[enabledKey];
    var pageConditions = this.state.pageConditions;
    this.props.dispatch({
      type:`${this.namespace}/reqToggleEnable`, 
      payload:{
        model:record,
        enabled:record[enabledKey]
      }, 
      pageConditions})
  }
